...
public class JoclTest03
{
    ...
    public JoclTest03(int width, int height)
    {
        screenWidth = width;
        screenHeight = height;
    ...
    private void initCL()
    {
        ...
        // Program Setup
        String source = readFile("resources/Kernels/TextureMethods/TextureTriangleParallel.cl");
    ...
    private void drawTriangle(double[][] triangle, double[][] textureAnchor)
    {
        ...
        if (Math.signum(v01[1]) != Math.signum(v02[1])) {
            ...
            height = Math.ceil(Math.abs(v04[1] * screenHeight));
            // Set work size and execute the kernel
            globalWorkSize = new long[]{(long)height};

            setupTriangleArgs(textureAnchor[2], t02, t03, triangle[2], v04, triangle[1], side, height);
            ...
            setupTriangleArgs(textureAnchor[1], t01, VectorCalc.mul_v_d(t03, -1), triangle[1], v04, triangle[2], side, height);
            clEnqueueNDRangeKernel(commandQueue, kernel, 1, null,
                    globalWorkSize, null, 0, null, null);
            ...
            height = Math.ceil(Math.abs(v02[1] * screenHeight));
            globalWorkSize = new long[]{(long)height};

            setupTriangleArgs(textureAnchor[0], t02, t01, triangle[0], v02, triangle[1], side, height);
            clEnqueueNDRangeKernel(commandQueue, kernel, 1, null,
                    globalWorkSize, null, 0, null, null);
            ...
            if (heightV01 != heightV02) {
                ...
                height = Math.ceil(Math.abs(v03[1] * screenHeight));
                globalWorkSize = new long[]{(long)height};

                setupTriangleArgs(textureAnchor[1], t03, t04, triangle[1], v03, triangle[0], side, height);
                clEnqueueNDRangeKernel(commandQueue, kernel, 1, null,
                        globalWorkSize, null, 0, null, null);
            }
        }
            ...
            height = Math.ceil(Math.abs(v01[1] * screenHeight));
            globalWorkSize = new long[]{(long)height};

            setupTriangleArgs(textureAnchor[0], t01, t02, triangle[0], v01, triangle[2], side, height);
            clEnqueueNDRangeKernel(commandQueue, kernel, 1, null,
                    globalWorkSize, null, 0, null, null);
            //if the heights are the same then we've already went over this portion
            if (heightV01 != heightV02) {
                ...
                setupTriangleArgs(textureAnchor[2], t03, VectorCalc.mul_v_d(t02, -1), triangle[2], v03, triangle[0], side, height);
                clEnqueueNDRangeKernel(commandQueue, kernel, 1, null,
                        globalWorkSize, null, 0, null, null);
            }
        }
    ...
    )
    {
        ...
        //set height value
        clSetKernelArg(kernel, 10, Sizeof.cl_double, Pointer.to(new double[]{height}));
        //set direction value
        clSetKernelArg(kernel, 11, Sizeof.cl_double, Pointer.to(new double[]{side}));
    }
    ...
    private void updateImage() {
        ...
        clSetKernelArg(kernel, 12, Sizeof.cl_mem, Pointer.to(debugBufferMem));
