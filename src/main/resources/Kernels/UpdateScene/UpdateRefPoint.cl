...
)
{
    ...
    double in[3] = {
        relativeArray[(n*3)+0], relativeArray[(n*3)+1], relativeArray[(n*3)+2]
    };
    double out[3];
    ...
    for (int i=0;i<3;i++)
    {
        clipArray[(n*3) + i] = out[i];
    }

    //transform out into screen-space
    screenArray[(n*3)+0] = (out[0] + 1) * 0.5 * screenSize[0];
    screenArray[(n*3)+1] = (1 - (out[1] + 1) * 0.5) * screenSize[1];
    screenArray[(n*3)+2] = out[2];
}
