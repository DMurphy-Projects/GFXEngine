...
__kernel void calculateMapInfo(
    ...
    __global double *pointArray, __global int *polygonStartArray, __global int *polygonArray,
...
)
{
    ...
    for (int _i=0;_i<n;_i++)
    {
        ...
        if (x > bx && x < (bx+width) && y > by && y < (by+height))
        {
            ...
            double p1x = pointArray[polygonArray[polygonOffset + 0]]; double p1y = pointArray[polygonArray[polygonOffset + 1]];
            ...
            {
                double p2x = pointArray[polygonArray[polygonOffset + (ii*3) + 0]];
                double p2y = pointArray[polygonArray[polygonOffset + (ii*3) + 1]];

                double p3x = pointArray[polygonArray[polygonOffset + ((ii+1)*3) + 0]];
                double p3y = pointArray[polygonArray[polygonOffset + ((ii+1)*3) + 1]];
                ...
                if ((w >= 0) && (v >= 0) && (u >= 0))
                {
                    double p1z = pointArray[polygonArray[polygonOffset + 2]];
                    double p2z = pointArray[polygonArray[polygonOffset + (ii*3) + 2]];
                    double p3z = pointArray[polygonArray[polygonOffset + ((ii+1)*3) + 2]];
